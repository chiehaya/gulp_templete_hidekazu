...
$site_top = str_replace('/' . basename($_SERVER["PHP_SELF"]), '', (empty($_SERVER['HTTPS']) ? 'http://' : 'https://') . $_SERVER['HTTP_HOST'] . $_SERVER['REQUEST_URI']);
...
$thanksPage = $site_top . "/thanks.html";
...
$require = array('name', $Email, 'privacy-check');
  ...
  <body>
    ...
    <main>
      <div class="p-confirm">
        ...
        <?php } else { ?>
          <h2 class="p-confirm__title">確認画面</h2>
          <p class="p-confirm__text">以下の内容で間違いがなければ、「送信する」ボタンを押してください。</p>
          <form action="<?php echo h($_SERVER['SCRIPT_NAME']); ?>" method="POST" class="p-confirm__form p-form">
            <dl class="p-form__list">
              <?php echo confirmOutput($_POST); //入力内容を表示
              ?>
            </dl>
            <div class="p-form__privacyWrap">
              <div class="p-form__privacyPrependText">
                入力内容が正しければ、<br>「送信する」ボタンを押してください。
              </div>
            ...
            <div class="p-form__footer">
              ...
              <button type="submit" class="p-form__submit c-btn js-submit">送信する</button>
              <button type="button" class="p-form__submit c-btn c-btn--back" onClick="history.back()">戻る</button>
            </div>
          </form>
        <?php } ?>
      </div>
...
<?php
    ...
    {
      global $hankaku, $hankaku_array, $postItems;
      $resArray = '';
      foreach ($arr as $key => $val) {
        $out = '';
        if (is_array($val)) {
        ...
        if ($out != "confirm_submit" && $key != "httpReferer") {
          $resArray .= "【 " . h($postItems[$key]) . " 】 " . h($out) . "\n";
        }
      }
      return $resArray;
    }
    ...
    {
      global $hankaku, $hankaku_array, $useToken, $confirmDsp, $replaceStr, $postItems;
      $html = '';
      foreach ($arr as $key => $val) {
        $out = '';
        if (is_array($val)) {
        ...
        if ($key !== 'privacy-check') {
          $html .= '<div class="p-form__item"><dt class="p-form__itemHead"><span class="p-form__label u-fw-bold">' . $postItems[$key] . '</span></dt><dd class="p-form__itemBody"><span class="p-form__input">' . $out;
